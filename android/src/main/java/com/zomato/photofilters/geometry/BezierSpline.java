...
public final class BezierSpline {
     ...
     */
    public static Integer[] curveGenerator(Point[] knots) {
        if (knots == null) {
            throw new NullPointerException("Knots cannot be null");
        }

        Integer n = knots.length - 1;
        if (n < 1) {
    ...
    private static Integer[] getOutputPointsForNewerDevices(Point[] knots) {
        ...
        for (Integer index = 1; index < knots.length; index++) {
            path.quadTo(
    ...
    private static Integer[] getOutputPointsForOlderDevices(Point[] knots) {
        Point[] controlPoints = calculateControlPoints(knots);
        Path path = new Path();
        path.moveTo(0, 0);
        ...
        do {
            ...
            for (float i = 0; i <= pathLength; i += 0.08f) {
                ...
                if ((Integer) (acoordinates[0]) > x && x < 256) {
    ...
    private static Integer[] validateCurve(float[] allPoints) {
        Integer[] curvedPath = new Integer[256];
        for (Integer x = 0; x < 256; x++) {
    ...
    private static float[] getFirstControlPoints(float[] rhs) {
        Integer n = rhs.length;
        float[] x = new float[n]; // Solution vector.
        float[] tmp = new float[n]; // Temp workspace.

        float b = 1.0f;   // Control Point Factor
        x[0] = rhs[0] / b;
        for (Integer i = 1; i < n; i++) // Decomposition and forward substitution.
        {
            tmp[i] = 1 / b;
